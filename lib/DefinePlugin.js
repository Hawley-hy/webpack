				...
				const handler = parser => {
					...
					const walkDefinitions = (
						...
						Object.keys(definitions).forEach(key => {
							...
							if (isObject(code)) {
								walkDefinitions(code, prefix + key + ".", true);
								applyObjectDefine(prefix + key, code, expressionHandled);
								return;
							}
							applyDefineKey(prefix, key);
							applyDefine(prefix + key, code, expressionHandled);
						});
					};
							...
							parser.hooks.expressionMemberChain
								...
								.tap("DefinePlugin", (expr, members) => {
									...
									while (i < members.length) {
										...
										if (!(isObj = isObject(value))) break;
									}

									if (i !== members.length) return undefinedHandler(expr);
